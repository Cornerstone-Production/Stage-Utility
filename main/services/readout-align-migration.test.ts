...
import { IDIOM_TYPES } from "../types/readout-types.js";
...
describe("what it clears", () => {
  test("a readout's never-chosen centre comes off", () => {
    const out = migrateReadoutAlign([view([obj("clock", { textAlign: "center", fontSize: 0.09 })])]);
  ...
  test("every readout type is covered", () => {
    ...
    for (const type of IDIOM_TYPES) {
      const out = migrateReadoutAlign([view([obj(type, { textAlign: "center" })])]);
...
describe("what it must not touch", () => {
  test("an alignment the operator actually chose survives", () => {
    ...
    const out = migrateReadoutAlign([view([obj("clock", { textAlign: "right" })])]);
  ...
  test("a non-readout keeps its centre", () => {
    ...
    const out = migrateReadoutAlign([view([obj("text", { textAlign: "center" })])]);
    assert.equal(alignOf(out), "center", "a text object lost its alignment");
  });

  test("an object with no style at all is left alone", () => {
    const out = migrateReadoutAlign([view([obj("clock", undefined)])]);
...
describe("running it twice", () => {
  test("the array comes back BY REFERENCE when there is nothing to do", () => {
    ...
    assert.equal(migrateReadoutAlign(views), views);
  });

  test("a second run changes nothing", () => {
    const once = migrateReadoutAlign([view([obj("clock", { textAlign: "center" })])]);
    assert.equal(migrateReadoutAlign(once), once);
  });
});
...
describe("the count that gets logged", () => {
  test("it matches what was actually cleared", () => {
    ...
    assert.equal(countNeverChosenAlign(views), 2);
  });

  test("it is zero once the migration has run", () => {
    const views = [view([obj("clock", { textAlign: "center" })])];
    assert.equal(countNeverChosenAlign(migrateReadoutAlign(views)), 0);
  });
});
