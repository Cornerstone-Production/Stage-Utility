...
//
// PURE: rules in, YAML text out — with one exception, a `[cues]` warning for a
// pair whose two halves press the same button with no state variable. That
// configuration cannot be generated by anything here, so the only place it can
// be noticed is where it is read. Generated rather than written by hand because
...
export function homeAssistantYaml(rules: Rule[], baseUrl: string): string {
  ...
  if (pairs.length > 0) {
    ...
    for (const p of pairs) {
      // A toggle pair is an ordinary switch — the state template is what makes
      // it one — but the config has to say why both directions call a
      // rest_command that presses the same key, or the next person to read it
      // takes it for a copy-paste mistake and "fixes" it.
      if (p.toggle && p.binding) {
        lines.push(
          "      # toggle button: both directions press the same Companion button, so the state variable is what tells them apart",
        );
      } else if (p.toggle) {
        // The import cannot produce this — a single button without a variable
        // stays a single cue — so it is two cues somebody wrote by hand, and it
        // is a switch that will report whatever it last asked for while the
        // device does the opposite every other press.
        lines.push(
          "      # WARNING: both halves press the same button and no state variable is bound — Home cannot know which way it went",
        );
        console.warn(`[cues] pair ${scrub(p.base)} presses one button with no state variable`);
      }
      lines.push(
