...
describe("the reserved list covers every path the router handles", () => {
  test("every slug literal compared in root-view.tsx is reserved", () => {
    ...
    assert.ok(compared.length >= 4, `expected route literals in root-view.tsx, found ${compared.length}`);
