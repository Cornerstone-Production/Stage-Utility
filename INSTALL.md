# Installing Stage Monitor on a server

Stage Monitor runs as a small web/server-only app: a Node backend serves the
...
- The repo cloned onto the server, e.g. in `/opt/stage-monitor/mic-display`.
...
3. Creates the data directory (default `/var/lib/stage-monitor`) and gives it to
   the service user.
4. Writes and enables a `stage-monitor` **systemd** service that starts on boot.
5. Prints the access URLs.

### Options

```bash
sudo ./scripts/install.sh --data-dir /srv/stage-monitor   # custom data dir
sudo ./scripts/install.sh --user stagemon                 # run as a specific user
sudo ./scripts/install.sh --no-service                    # build only, no systemd
```

`STAGE_MONITOR_DATA=/path` works as an alternative to `--data-dir`. The service
...
```bash
systemctl status stage-monitor       # is it running?
journalctl -u stage-monitor -f       # tail logs
sudo systemctl restart stage-monitor # restart
```
...
```bash
cd /opt/stage-monitor/mic-display
git pull
...
Back up the **data directory** (`/var/lib/stage-monitor` by default). It holds
