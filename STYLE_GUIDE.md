...
|---|---|
| **Button** | variants `accent` (`bg-accent text-white` → `hover:bg-accent-hover`), `filled` (`bg-fill text-fg`), `transparent` (`text-fg-muted hover:bg-fill`); sizes `small` (h-6, `text-caption1`) / `medium` (h-8, `text-footnote`); `rounded-md font-medium`; ring `ring-focus`. Icon-only auto-tooltips from `aria-label`. Icon-only buttons carry the `.touch-target` utility (styles.css), which extends the tap area to 44×44px under `(pointer: coarse)` without changing the drawn size; a mouse sees no difference. Where two or more icon-only buttons sit within ~8px of each other in a row, use `.touch-target-y` instead — it grows the tap area to 44px tall only, staying the host's own width, so neighbouring buttons' enlarged targets cannot overlap and steal each other's taps. |
...
| **NumberInput** | input + chevron steppers (`ChevronUp/Down`), hides native spinner, `tabular-nums`, optional `suffix`; commits live on change **and** on each stepper press (`onChange` fires for dirty-tracking, `onCommit` on settle). Holding a stepper repeats it — once immediately, then every 80ms after a 400ms delay — for a mouse and a finger alike. The convention for all numeric fields — never a raw `<input type="number">`. |
