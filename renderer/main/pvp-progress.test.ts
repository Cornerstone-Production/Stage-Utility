...
describe("computePvpProgress", () => {
  test("at the moment of the sample it is the anchor exactly", () => {
    const p = computePvpProgress(layer(), T, AT);
    assert.ok(p);
  ...
  test("it advances locally between frames, which is the whole point", () => {
    ...
    // cost the feature.
    const p = computePvpProgress(layer(), T, AT + 3000);
    assert.ok(p);
  ...
  test("it never runs past the end, however stale the anchor", () => {
    ...
    const p = computePvpProgress(layer(), T, AT + 600_000);
    assert.ok(p);
  ...
  test("a paused clip does not advance", () => {
    ...
    const p = computePvpProgress(layer({ playbackRate: 0 }), T, AT + 5000);
    assert.ok(p);
    assert.equal(p.elapsedSec, 10);
  });

  test("a still has no progress at all", () => {
    assert.equal(computePvpProgress(layer({ state: "still", durationSec: null }), T, AT), null);
  });

  test("an empty layer has no progress at all", () => {
    assert.equal(
      computePvpProgress(layer({ state: "empty", anchorElapsedSec: null, durationSec: null }), T, AT),
      null,
    );
  });
  ...
  test("a null or unparseable sampledAt yields no progress, never NaN", () => {
    for (const at of [null, "", "not a date"]) {
      assert.equal(computePvpProgress(layer(), at, AT), null, `sampledAt ${String(at)} produced a reading`);
    }
  });

  test("a zero or negative duration yields no progress, never a divide by zero", () => {
    assert.equal(computePvpProgress(layer({ durationSec: 0 }), T, AT), null);
    assert.equal(computePvpProgress(layer({ durationSec: -5 }), T, AT), null);
  });
});
...
describe("stillOnScreenSec", () => {
  test("a still with a known arrival counts up from it", () => {
    assert.equal(stillOnScreenSec(still(), AT), 20);
    assert.equal(stillOnScreenSec(still(), AT + 5000), 25);
  });

  test("null for anything that is not a still", () => {
    assert.equal(stillOnScreenSec(layer({ state: "video", mediaSinceAt: STILL_SINCE }), AT), null);
    assert.equal(stillOnScreenSec(layer({ state: "empty", mediaSinceAt: null }), AT), null);
  });

  test("null when PVP has not told us an arrival time yet", () => {
    assert.equal(stillOnScreenSec(still({ mediaSinceAt: null }), AT), null);
  });
});

describe("computeStillProgress", () => {
  test("a hold from the PVP integration card counts a still down", () => {
    const p = computeStillProgress(still(), AT, 20);
    assert.ok(p);
    assert.equal(p.elapsedSec, 20);
    assert.equal(p.remainingSec, 0);
    assert.equal(p.fraction, 1);
  ...
  test("THE GUARD: a widget's own hold overrides the card default, not the other way round", () => {
    ...
    const halfway = computeStillProgress(still(), AT, 30);
    assert.ok(halfway);
    assert.equal(halfway.remainingSec, 10);
    assert.notEqual(halfway.remainingSec, computeStillProgress(still(), AT, 20)?.remainingSec);
  });

  test("remaining clamps at 0 rather than going negative", () => {
    const p = computeStillProgress(still(), AT + 60_000, 20);
    assert.ok(p);
    assert.equal(p.remainingSec, 0);
    assert.equal(p.elapsedSec, 20);
    assert.equal(p.fraction, 1);
  ...
  test("no hold configured (null or non-positive) yields no progress", () => {
    assert.equal(computeStillProgress(still(), AT, null), null);
    assert.equal(computeStillProgress(still(), AT, 0), null);
    assert.equal(computeStillProgress(still(), AT, -5), null);
  });

  test("no mediaSinceAt yields no progress, whatever the hold", () => {
    assert.equal(computeStillProgress(still({ mediaSinceAt: null }), AT, 20), null);
  });

  test("a video layer never gets a still's progress", () => {
    assert.equal(computeStillProgress(layer(), AT, 20), null);
  });
});
