// Colour for a ScriptView note category, when a layout tints rows by category rather
// than by PCO's item colours.
//
// A fixed keyword table with no configuration. PCO has no colour for a note category —
...
// grey. Acceptable, because the categories that matter on a stage rundown — audio,
...
/** Colour for a note category. Matching is case-insensitive and by substring, so
