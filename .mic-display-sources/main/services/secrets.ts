// Encryption-backend-agnostic secret store. Each integration gets one secret slot.
// Secrets are stored as an encrypted JSON blob at userData/secrets.bin.
//
// In Glaze mode, index.ts wires safeStorage as the backend.
// In standalone mode, server.ts wires an AES-256-GCM backend.
