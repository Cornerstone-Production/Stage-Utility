...
export class RemoteServer {
  ...
  constructor() {
    // Resolve control.html with a multi-candidate fallback so it works in
    // build, dev (tsx watch), and standalone mode.
    this._controlHtmlCandidates = [
      path.join(__dirname, "..", "control.html"),           // Glaze build
      path.join(__dirname, "..", "..", "control.html"),     // tsx dev
      path.join(process.cwd(), "control.html"),             // standalone cwd
      path.join(process.cwd(), "public", "control.html"),  // standalone public/
    ];
  }
  ...
  private async handleRequest(
    ...
    if (method === "GET" && !pathname.startsWith("/api/") && pathname !== "/photos") {
      ...
      // Fall through to control page (legacy / Glaze mode)
    }
