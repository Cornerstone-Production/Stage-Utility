...
- [ ] No secret anywhere in the diff: `git diff origin/beta... | grep -inE "bearer [a-z0-9]|token.*=.*['\"][a-z0-9]{8}"` returns nothing, and no LAN address: `git diff origin/beta... | grep -nE "192\.168\.|10\.[0-9]+\.|172\.(1[6-9]|2[0-9]|3[01])\."` returns nothing except addresses in `192.168.1.0/24`, which is the documentation range this repo writes examples and placeholders in — the ProVideoPlayer descriptor's `192.168.1.50` matches the OBS and REAPER descriptors already there. Anything OUTSIDE that range is a real box: `main/` still carries `192.168.16.61` and `192.168.16.74`, the church's own server and a kiosk, across six files, and they want replacing with the documentation range.
- [ ] No church name, no real service-type id, no real layer or media name and no LAN address anywhere in the repo, **`docs/` included**: `git diff origin/beta... | grep -inE "cornerstone|series graphic|youthkickoff" | grep -viE "cornerstone-production|cornerstone-stageutility|Cornerstone Stage Utility|com\.cornerstone\."` returns nothing, and so does `grep -rniE "cornerstone|scriptviewer\.io|salt company" docs/ README.md | grep -viE "cornerstone-production|cornerstone-stageutility|Cornerstone Stage Utility|com\.cornerstone\.|the cornerstone"`. It used to be scoped to `main/ renderer/` on the reasoning that a research doc records real observed names as evidence, "which is its job" — but this repository is PUBLIC, so `docs/` publishes them exactly as widely as code would. That scoping is how the calendar plan shipped the real PCO calendar, ministry and resource names, and how two of the church's own LAN addresses reached a spec. The exclusions above are the GitHub org, the Homebrew tap, the Companion module name, the launchd label and the English word.
